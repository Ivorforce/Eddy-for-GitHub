...
**Timeline popover (`templates/_row.html` + `_timeline_event.html`):** chronological conversation log, rendered in both modes — `_attach_timeline` runs unconditionally. User chat and reply-bearing AI verdicts render as full chat bubbles (AI purple-right with the standing `description` as an aside under the `reply`; user blue-left); a reply-less verdict collapses to a compact one-liner like the comment/review lines. Adjacent dismissal user_actions are coalesced — see `_coalesce_user_actions`. **AI mode only:** a composer at the bottom with two acts — Enter saves a free Note (`/ai/<id>/chat`, no AI call), Ctrl+Enter sends to the AI (`/ai/<id>/judge` with the text — "↻ Re-ask" when empty, "Send to AI" with text). Defaulting Enter to the free Note matches the cost asymmetry (a judgment is ~0.5¢ + latency; a note costs nothing and the AI picks it up next time it judges).
