...
`action_now` is a suggestion for what the user should do — `look` (open the link), `ignore` (mark read, row stays visible), `mute` (full opt-out — unsubscribe, never resurfaces), `archive` (the thread is *finished* — no more activity expected; not a "park it for now", since Done auto-resurfaces on the next activity), `snooze` (hide until ~`snooze_days` out). The "stay subscribed but quiet the churn" middle ground is `subscription_changes`, not `action_now`. `subscription_changes` is a forward-looking, advisory delta on the thread's `muted_kinds` (which the AI now sees in its input); the per-thread tool schema only offers the kinds that fire on that type, in the right mute/unmute direction. `reply` is the one non-standing field — a conversational answer when a `user_chat` asked something; the prompt tells the AI to omit it rather than emit filler. Nothing auto-applies. The verdict cache (`notifications.ai_verdict_*`) is never auto-cleared — Re-ask overwrites it (and `_save_verdict` keeps the prior verdict in `thread_events` for the next judgment to see).
...
**Timeline popover (`templates/_row.html` + `_timeline_event.html`):** chronological conversation log, rendered in both modes — `_attach_timeline` runs unconditionally. AI verdicts and user chat render as full chat bubbles (AI purple-right, user blue-left); in a verdict the standing `description` is always a recessed aside, and a `reply`, when present, sits above it as the body. Adjacent dismissal user_actions are coalesced (`_coalesce_user_actions`) and runs of `body_edit` collapse to the latest (`_coalesce_body_edits`). **AI mode only:** a composer at the bottom with two acts — Enter saves a free Note (`/ai/<id>/chat`, no AI call), Ctrl+Enter sends to the AI (`/ai/<id>/judge` with the text — "↻ Re-ask" when empty, "Send to AI" with text). Defaulting Enter to the free Note matches the cost asymmetry (a judgment is ~0.5¢ + latency; a note costs nothing and the AI picks it up next time it judges).
